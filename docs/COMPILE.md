...
- Uses `native\fastglob.def` for JNI exports
- Outputs to `build\fastglob.dll`

The Maven `pom.xml` will automatically pick up `build\fastglob.dll` and bundle it inside the JAR.

## JNI Exports (.def File)

When using JNI, you MUST export your native functions in the `native\fastglob.def` file:

```def
LIBRARY fastglob
EXPORTS
    Java_fastglob_FastGLOB_doSomethingNative
```
